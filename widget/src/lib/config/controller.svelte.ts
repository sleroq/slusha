...
export class ConfigController {
    ...
    chatOverrideConfig = $state(fromUnknownChatOverride({}, defaultGlobalConfig()));
    chatBaseConfig = $state<ResolvedChatOverridePayload>(fromUnknownChatOverride({}, defaultGlobalConfig()));
    ...
    constructor() {
        ...
        const chatIdFromStartParam = parseChatIdFromStartParam(params.get('tgWebAppStartParam'));
    ...
    get canSaveChat(): boolean {
        return Boolean(this.bootstrap?.canEditChat) && this.chatId.trim().length > 0;
    }
    ...
    #hydrateForms(data: BootstrapResponse): void {
        ...
        this.chatOverrideConfig = fromUnknownChatOverride(data.chatOverridePayload, baseConfig);
    ...
    async saveGlobal(): Promise<void> {
        ...
        this.status = result.ok ? 'Global config saved' : (result.error ?? 'Failed to save global config');
    }
    ...
    async saveChat(): Promise<void> {
        ...
        const payload = buildChatPayload(this.chatOverrideConfig, this.chatText, this.chatBaseConfig);
        const result = await saveChatConfig(chatId, payload, rawInitData);
        this.status = result.ok ? 'Chat override saved' : (result.error ?? 'Failed to save chat override');
    }
}
