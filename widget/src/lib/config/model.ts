...
export function defaultChatEditableAiConfig(source?: Partial<AiPayload>): ChatEditableAiPayload {
    const base = source ?? {};
    return {
        ...
        privateChatPromptAddition: typeof base.privateChatPromptAddition === 'string'
        ...
        commentsPromptAddition: typeof base.commentsPromptAddition === 'string'
            ...
            : '',
        hateModePrompt: typeof base.hateModePrompt === 'string' ? base.hateModePrompt : '',
        useJsonResponses: typeof base.useJsonResponses === 'boolean' ? base.useJsonResponses : true,
        messagesToPass: typeof base.messagesToPass === 'number' ? base.messagesToPass : 5,
        messageMaxLength: typeof base.messageMaxLength === 'number' ? base.messageMaxLength : 4096,
        includeAttachmentsInHistory: typeof base.includeAttachmentsInHistory === 'boolean'
            ? base.includeAttachmentsInHistory
            : true,
        bytesLimit: typeof base.bytesLimit === 'number' ? base.bytesLimit : 20 * 1024 * 1024,
    };
}
...
export function fromUnknownGlobal(payload: unknown): UserConfigPayload {
    ...
    const obj = payload && typeof payload === 'object' ? (payload as Partial<UserConfigPayload>) : {};
    const ai = obj.ai && typeof obj.ai === 'object' ? (obj.ai as Partial<AiPayload>) : {};
    ...
    return {
        ...
        trustedIds: Array.isArray(obj.trustedIds)
            ? obj.trustedIds.filter((item): item is number => typeof item === 'number')
            : [],
        availableModels: normalizeModels(obj.availableModels),
    };
}

export function fromUnknownChatOverride(payload: unknown, global: UserConfigPayload): ResolvedChatOverridePayload {
    const obj = payload && typeof payload === 'object' ? (payload as ChatOverridePayload) : {};
    ...
    return {
        ...
        tendToReplyProbability: obj.tendToReplyProbability ?? global.tendToReplyProbability,
        tendToIgnore: obj.tendToIgnore ?? global.tendToIgnore,
        tendToIgnoreProbability: obj.tendToIgnoreProbability ?? global.tendToIgnoreProbability,
        randomReplyProbability: obj.randomReplyProbability ?? global.randomReplyProbability,
        nepons: obj.nepons ?? global.nepons,
        responseDelay: obj.responseDelay ?? global.responseDelay,
    };
}

export function globalTextFromConfig(config: UserConfigPayload): GlobalFormText {
    return {
...
export function chatTextFromConfig(config: ResolvedChatOverridePayload): ChatFormText {
    return {
        names: matcherListToTextarea(config.names ?? []),
        tendToReply: matcherListToTextarea(config.tendToReply ?? []),
        tendToIgnore: matcherListToTextarea(config.tendToIgnore ?? []),
...
export function buildGlobalPayload(config: UserConfigPayload, text: GlobalFormText): UserConfigPayload {
    return {
        ...config,
...
function equalMatcherList(a: Matcher[], b: Matcher[]): boolean {
    ...
    return a.every((item, index) => matcherToLine(item) === matcherToLine(b[index]));
}
...
export function buildChatPayload(
    ...
    if (!equalMatcherList(tendToReply, base.tendToReply)) payload.tendToReply = tendToReply;
    if (!equalMatcherList(tendToIgnore, base.tendToIgnore)) payload.tendToIgnore = tendToIgnore;
    ...
    if (config.ai.temperature !== base.ai.temperature) aiPayload.temperature = config.ai.temperature;
    if (config.ai.topK !== base.ai.topK) aiPayload.topK = config.ai.topK;
    if (config.ai.topP !== base.ai.topP) aiPayload.topP = config.ai.topP;
    if ((config.ai.prompt ?? '') !== (base.ai.prompt ?? '')) aiPayload.prompt = config.ai.prompt;
    if ((config.ai.dumbPrompt ?? '') !== (base.ai.dumbPrompt ?? '')) {
        aiPayload.dumbPrompt = config.ai.dumbPrompt;
    }
    if ((config.ai.privateChatPromptAddition ?? '') !== (base.ai.privateChatPromptAddition ?? '')) {
        aiPayload.privateChatPromptAddition = config.ai.privateChatPromptAddition;
    }
    if ((config.ai.groupChatPromptAddition ?? '') !== (base.ai.groupChatPromptAddition ?? '')) {
        aiPayload.groupChatPromptAddition = config.ai.groupChatPromptAddition;
    }
    if ((config.ai.commentsPromptAddition ?? '') !== (base.ai.commentsPromptAddition ?? '')) {
    ...
    if (config.ai.includeAttachmentsInHistory !== base.ai.includeAttachmentsInHistory) {
        aiPayload.includeAttachmentsInHistory = config.ai.includeAttachmentsInHistory;
    }
