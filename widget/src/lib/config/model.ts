...
function normalizeGenerationTaskConfig(
    ...
    if (!value || typeof value !== 'object') {
        return {
            ...base,
            thinking: normalizeThinkingConfig(base.thinking),
        };
    }
    ...
    return {
        ...base,
        ...obj,
        thinking: normalizeThinkingConfig(obj.thinking ?? base.thinking),
    };
}
...
export function defaultChatEditableAiConfig(
    ...
    return {
        ...
        temperature: typeof base.temperature === 'number'
            ? base.temperature
            : 0.8,
        ...
        privateChatPromptAddition:
            typeof base.privateChatPromptAddition === 'string'
                ...
                : '',
        groupChatPromptAddition:
            typeof base.groupChatPromptAddition === 'string'
...
export function fromUnknownGlobal(payload: unknown): UserConfigPayload {
    ...
    return {
        ...
        nepons: Array.isArray(obj.nepons)
            ? obj.nepons.filter((item): item is string =>
                typeof item === 'string'
            )
            : [],
        adminIds: Array.isArray(obj.adminIds)
            ? obj.adminIds.filter((item): item is number =>
                typeof item === 'number'
            )
            : [],
